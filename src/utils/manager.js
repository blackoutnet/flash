 ...
 * @returns {string|null}
 */
export const checkCompatibleDevice = (storageInfo) => {
        ...
        for (const [slot, onSlotProgress] of withProgress(slots, onFlash)) {
          ...
          if (
            !(await this.device.flashBlob(
              partitionName,
              blob,
              (progress) => onSlotProgress(progress / image.size),
              false
            ))
          ) {
...
/**
 * Factory function for creating and initializing a FlashManager instance
 *
 * @param {Object} options - Configuration options
 * @param {string} options.manifestUrl - URL to the manifest file
 * @param {string} options.programmerUrl - URL to the programmer binary
 * @param {FlashManagerCallbacks} options.callbacks - Event callbacks
 * @returns {Promise<FlashManager>} Initialized FlashManager instance
 * @throws {Error} If initialization fails
 */
export const createFlashManager = async ({
  manifestUrl,
  programmerUrl,
  callbacks = {},
}) => {
  try {
    // Fetch the programmer binary
    const response = await fetch(programmerUrl);
    if (!response.ok) {
      throw new Error(
        `Failed to fetch programmer binary: ${response.status} ${response.statusText}`
      );
    }
    const programmer = await response.arrayBuffer();

    // Create the FlashManager instance
    const flashManager = new FlashManager(manifestUrl, programmer, callbacks);

    // Create and initialize ImageManager
    const imageManager = new ImageManager();

    // Initialize the FlashManager with the ImageManager
    await flashManager.initialize(imageManager);

    // Check if initialization was successful (no errors set)
    if (flashManager.error !== ErrorCode.NONE) {
      throw new Error(
        `FlashManager initialization failed with error code: ${flashManager.error}`
      );
    }
    ...
    console.error(
      "[FlashManagerFactory] Failed to create FlashManager:",
      error
    );
    throw error;
  }
